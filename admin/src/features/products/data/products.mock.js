export const productsMock = [
  {
    id: 1,
    name: 'أرز مصري ممتاز المطبخ',
    slug: 'egyptian-rice',
    image:
      'https://lh3.googleusercontent.com/aida-public/AB6AXuCMqCiKA0yMYsFnJpRBYVLduxWhPM24XDVKfS5K3WUQRsmhejGeXlTa3PY5tYExeRuKyRb3kzM53zNH9PbxnfD6ZQUgrexhMNEyyLEVjVRQlVgqhOYOjqUbX6W4Dcy3YTTcKc6etRwsTr7C7d9wASL6l4eRkvB7IWs0PlkD9CKbqo-Bvrp9R38FBMZP7W_YyQK1Knj9VCrUa7RKnkfB9Fk2XlrZ_9adVqi8xOQlMByNrjqndrDKwSKG',
    unit: '1 كيلو',
    price: 39,
    discountPercentage: 10,
    description:
      'أرز مصري عالي الجودة، مناسب للاستخدام اليومي ويتميز بحبات متماسكة وطعم مميز.',
    badge: 'مميز',
    isBestSeller: true,
    rating: 4.6,
    reviewsCount: 128,
    deliveryText: 'يصل خلال 30 دقيقة',
    categorySlug: 'grocery',
    subcategoryId: 1,
    stock: 50,
    isActive: true,
  },
  {
    id: 2,
    name: 'مكرونة اسباجيتي إيطاليانو',
    slug: 'italiano-spaghetti',
    image:
      'https://lh3.googleusercontent.com/aida-public/AB6AXuDBTkNJUmFjplJAwFYe3i4QodzkHeRy7SABtvacIdvdKiR8DUjG_42exS4J-GjlC0POYVrhsnFdh9-w4ExzCFdu8u8MdG9JjgpgmhTST05LZaNs3tqtnbICIaeq4M-HXVfngnE1RJfJs2d3WKVsdLtqanyx3HNa9-WHoRRvdr39of9ol2ylyUwPXspEfX3IZsyqQ949I3ja-G0_XT-CLiZFDt2dB1VhhutFVX_1ttHwTELJnWLW7wQX',
    unit: '400 جرام',
    price: 14,
    discountPercentage: 0,
    description:
      'مكرونة اسباجيتي بجودة عالية، مناسبة للوجبات اليومية وسهلة التحضير.',
    badge: null,
    isBestSeller: true,
    rating: 4.4,
    reviewsCount: 75,
    deliveryText: 'يصل خلال 30 دقيقة',
    categorySlug: 'grocery',
    subcategoryId: 1,
    stock: 35,
    isActive: true,
  },
   {
    id: 3,
    name: 'شيبسي',
    slug: 'italiano-spaghetti',
    image:
      'https://lh3.googleusercontent.com/aida-public/AB6AXuDBTkNJUmFjplJAwFYe3i4QodzkHeRy7SABtvacIdvdKiR8DUjG_42exS4J-GjlC0POYVrhsnFdh9-w4ExzCFdu8u8MdG9JjgpgmhTST05LZaNs3tqtnbICIaeq4M-HXVfngnE1RJfJs2d3WKVsdLtqanyx3HNa9-WHoRRvdr39of9ol2ylyUwPXspEfX3IZsyqQ949I3ja-G0_XT-CLiZFDt2dB1VhhutFVX_1ttHwTELJnWLW7wQX',
    unit: '100 جرام',
    price: 15,
    discountPercentage: 0,
    description:
      'شيبسي شطه ولمون بيخلي القلب يهون',
    badge: null,
    isBestSeller: true,
    rating: 4.4,
    reviewsCount: 75,
    deliveryText: 'يصل خلال 30 دقيقة',
    categorySlug: 'snacks',
    subcategoryId: 3,
    stock: 50,
    isActive: true,
  },
]